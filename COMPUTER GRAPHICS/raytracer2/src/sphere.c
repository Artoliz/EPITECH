...
** Last update Wed May 24 14:49:36 2017 Charles Aubert
*/

#include "../inc/src.h"

double	check_return_sphere(double k1, double k2)
{
  if (k1 < 0.0 && k2 < 0.0)
    return (-1.0);
  else if (k1 < 0.0)
...
double		intersect_sphere(sfVector3f e, sfVector3f v,
				 double r)
{
  double	a;
  double	b;
  ...
  b = 2 * (e.x * v.x + e.y * v.y + e.z * v.z);
  c = pow(e.x, 2) + pow(e.y, 2) + pow(e.z, 2) - pow(r, 2);
  delta = pow(b, 2) - (4 * a * c);
  if (delta == 0.0 && a != 0.0)
    {
      k1 = -b / (2 * a);
  ...
  else if (delta > 0.0 && a != 0.0)
    {
      k1 = (-b + sqrt(delta)) / (2 * a);
      k2 = (-b - sqrt(delta)) / (2 * a);
      return (check_return_sphere(k1, k2));
    }
  else
    return (-1.0);
}
